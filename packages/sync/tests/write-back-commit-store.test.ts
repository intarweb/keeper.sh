...
const RETURNED_APPLIED_COUNT = 3;
      ...
      set: (values: Record<string, unknown>) => ({
        where: () => {
          ...
          return createResolvedWrite({
            writeBackAppliedCount: RETURNED_APPLIED_COUNT,
...
describe("the commit that ends a write-back", () => {
  ...
  it("spends both budgets and clears any pending deletion", async () => {
    ...
    /*
     * The runaway stop is judged on the landed-write count alone, so that is what the
     * commit has to hand back. Handing back the shared epoch instead makes the write that
     * lands once a provider throttle lifts read as the fifth write in a runaway.
     */
