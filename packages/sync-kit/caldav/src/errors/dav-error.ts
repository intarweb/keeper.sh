...
const decodeDavError = (thrown: unknown): DecodedDavError => {
  ...
  let depth = 0;
  while (depth <= maximumCauseDepth) {
    if (!isCarrier(current) || seen.has(current)) {
      return { status: null, precondition: null, retryAfter: null, depth };
    }
    seen.add(current);
    const status = statusIn(current);
    const precondition = preconditionIn(current);
    if (status !== null || precondition !== null) {
      return { status, precondition, retryAfter: instantIn(current), depth };
    }
