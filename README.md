...
| KEEPER_ECHO_COMPARISON         | `worker`, `cron` | Optional. How reconciliation judges a destination's read-back. `on` (default) compares it against what the destination reported storing, so a provider that rewrites what it was given settles after one correction. `shadow` measures that verdict but keeps replacing on the old rule, and `off` disables it entirely.<br><br>e.g. `on`, `shadow`, `off` |
| KEEPER_ECHO_ADOPTION           | `worker`, `cron` | Optional. Set to `off` to stop recording read-back observations onto mappings. Defaults to `on`; turning it off leaves mirrors converging one cycle slower and is only useful for isolating database write load. |
| KEEPER_ECHO_ADOPTION_MAX_PER_RUN | `worker`, `cron` | Optional. Caps how many observations a single calendar sync records. Defaults to `2000`. |
