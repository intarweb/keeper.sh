...
const resolveIngestErrorSlug = (error: unknown): string => {
  if (error instanceof CalDAVPartialResponseError) {
    widelog.set("provider.objects_returned", error.returnedCount);
    widelog.set("provider.objects_missing_data", error.missingCount);
