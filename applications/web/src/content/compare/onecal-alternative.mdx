...
OneCal bundles several scheduling tools into one subscription. Keeper.sh does the sync job only, reaches calendars OneCal does not, and is free to start. If you would rather nobody else held your calendar, it is open source under AGPL-3.0 and runs on a machine you own.
...
Pick Keeper.sh if a calendar you rely on lives outside Google, Outlook and iCloud, for a free tier, and for event details that stay private by default. OneCal's extras are mobile apps, booking links and single sign-on for a team.
