...
description: "Keeper.sh is free forever and $5 flat at any number of calendars. Nine calendar sync tools compared on price, privacy defaults and what survives the vendor. Every figure verified 11 August 2026."
blurb: "Keeper.sh is free forever and $5 flat at any number of calendars. Nine calendar sync tools compared on price, privacy defaults and what survives the vendor."
...
- **Keeping your event titles to yourself — Keeper.sh.** A calendar you connect starts stripped: Keeper.sh writes a block of busy time carrying the calendar's name, and leaves the title, the description and the location behind. Pro is what switches detail on.
...
- **Regulated work.** CalendarBridge is the only tool here with a signed HIPAA BAA and Microsoft GCC High support, and Keeper.sh has neither today. That costs [$40 a month for eight calendars](https://calendarbridge.com/pricing/), US cloud only, nothing to run yourself.
- **AI week-planning.** Reclaim.ai has an AI scheduler Keeper.sh does not; sync there is one of eight features, rate-limited by tier, connecting [Google and Outlook only](https://help.reclaim.ai/en/articles/3600762-calendar-sync-overview-keep-multiple-schedules-in-sync).
- **A signed DPA on German hosting.** Kalender Sync publishes [Falkenstein databases, a signed GDPR agreement and named subprocessors](https://kalender-sync.de/en/it-admin/) from [€4 a month](https://kalender-sync.de/en/pricing/); self-hosted Keeper.sh puts the database on your own hardware, in your own country.

[Connect two accounts free](/register). There is no clock on it and no card at the end.
...
Four of the nine publish an MCP server, the connector that lets an assistant like Claude or ChatGPT act on your calendar rather than just talk about it.
...
| **CalendarBridge** | [Yes](https://help.calendarbridge.com/user-docs/bring-your-own-agent-mcp/), on every plan | Count not published; covers calendars, events, scheduling pages, syncs and connected accounts |

Keeper.sh's is free at 25 calls a day and uncapped on Pro, and the REST API behind it does the same things from a script. Checked 14 August 2026.

## What ten calendars cost on each tool
...
- **CalendarBridge** — [$40 a month, the top published tier, covering eight calendars](https://calendarbridge.com/pricing/). Ten calendars is not on their price list at all.
...
At two connected accounts Keeper.sh is free outright, where OneCal, [CalendarBridge](https://calendarbridge.com/pricing/) and [SyncThemCalendars](https://syncthemcalendars.com/) have no free plan at all. At eight calendars the gap is $35 a month, every month. Your calendars got messier; nothing about the product got better.

[Start with two accounts, free](/register) — the $5 tier is there when you outgrow it, at any number of calendars.
...
The code is [AGPL-3.0 on GitHub](https://github.com/ridafkih/keeper.sh), with seven published Docker images, including a standalone container that brings up the whole stack. The version you can run yourself is the version that runs the hosted service. [Start on the hosted version](/register) — same engine, none of the upkeep.

### SyncDate

SyncDate is a hosted service [run from Hetzner data centres in Germany and Finland](https://syncdate.app/security), with a [self-serve data processing agreement](https://syncdate.app/dpa). It publishes [about four seconds on Google and Outlook with a 15-minute backup check](https://syncdate.app/features), at [$8.99 a month](https://syncdate.app/pricing) — all checked 11 August 2026.
...
OneCal bills sync, booking links and a unified calendar view as one product, at [$5 per user a month for two calendars, up to $25 for fifty, both billed annually](https://www.onecal.io/pricing). It advertises ["real-time" with no figure attached](https://docs.onecal.io/docs/calendar-sync/calendar-sync-intro) and [up to 10 minutes for iCloud](https://www.onecal.io/features/apple-calendar-sync).
...
CalendarBridge is the only tool on this page with a HIPAA BAA (compliant since 2022, Premium plan up), SOC 2 Type 1, and Microsoft GCC High support for government tenants. Pricing runs [$5 a month for two calendars, up to $40 for eight](https://calendarbridge.com/pricing/), with [within a minute on Google and Outlook, and 5–10 minutes on iCloud](https://help.calendarbridge.com/faq/how-often-do-calendars-sync/) published.

**The trade-off** — $40 a month for eight calendars is the most expensive per-calendar price on this page, and the [published tiers stop there](https://calendarbridge.com/pricing/). Compliance is certificate-based on US cloud: no EU or regional residency, and no self-hosting.

### SyncThemCalendars

SyncThemCalendars charges [$3.99 a month for five sync pairs, rising to $17.99 for thirty-six](https://syncthemcalendars.com/), with unlimited calendars and events and [no speed figure published](https://syncthemcalendars.com/).
...
**The trade-off** — its ["Open source" checkmark links to no repository and names no license](https://unicala.io/), checked 11 August 2026. You can only own your data if you can read and run the software holding it. Ours is [on GitHub](https://github.com/ridafkih/keeper.sh) under AGPL-3.0, with the Docker images that build it.

### Reclaim.ai

Reclaim.ai is an AI that arranges your week, which Keeper.sh does not do. Sync is one of eight things it does, at [$10 per seat a month](https://reclaim.ai/pricing) with [one calendar sync on the free tier](https://reclaim.ai/pricing), and it publishes [no speed figure for sync](https://reclaim.ai/features/calendar-sync).
...
OGCS is free and open source under MPL-2.0, and runs on a Windows PC with desktop Outlook installed. It syncs [every 15 minutes, or every 2 minutes for changes you make in Outlook](https://www.outlookgooglecalendarsync.com/guide/syncoptions-when.html).
...
Almost everyone should let us run it: a server, a domain, updates, backups and your own Google and Microsoft sign-in apps are real work. Either way you can read exactly what the code does with your calendar before you trust it with one. [Start on the hosted version](/register).
...
[Connect two accounts free](/register)
...
Every competitor figure here comes from that vendor's own pages, checked on 11 August 2026, and CalendarBridge again on 14 August 2026. Keeper.sh's own figures come from the code that runs the hosted service, so you can check ours the same way we checked theirs.
