...
Morgen's calendar-blocking workflow "will run daily at 6:00 pm" and covers "the upcoming 14 days" ([Morgen's guide](https://www.morgen.so/guides/how-to-use-morgens-calendar-propagation-workflow)). The same guide is upfront about the consequence. When an event is cancelled or moved, "there will be a lag as the workflow runs daily at 6pm".
...
For filter rules that decide event by event, Morgen's controls go further than ours.
...
It is a real trade. Morgen's design holds no copy of your events, and ours holds one. The self-hosting section below puts that copy in a database you own.
...
Keeper.sh is a web app you open in a browser, set up once, and close. There is no Keeper.sh desktop or phone app.
...
Yes. The two do not conflict, because Morgen reads your calendars and Keeper.sh writes to them.

Some people keep Morgen as the calendar app, with Keeper.sh underneath doing the blocking.
...
Pick Keeper.sh if a stale calendar has already got you double-booked, or if your availability needs to be right within about a minute of a change. Pick Keeper.sh if you need your time blocked further out than the next two weeks.

Pick Keeper.sh if $5 a month, flat, is the right size for a background job.

Morgen's case is the app itself: native apps on desktop, phone and Linux, tasks beside your calendar, and EU or Swiss hosting.
...
Once a day. Morgen's guide states that the workflow runs daily at 6:00 pm and covers the upcoming 14 days. Keeper.sh reads every minute instead.
