...
You have calendars in more than one place and you want your busy time to show up in all of them. Keeper.sh, which costs $5 a month however many calendars you connect, and Kalender Sync, a Munich product, both do that job.
...
You create one for another person, they accept an invitation, and your blocks keep appearing in their calendar from then on. You pick from three visibility levels, from availability only, through title and time, up to full details. Their free plan includes one Sharing.
...
Kalender Sync's integrations page lists Google Calendar, Microsoft Outlook, Apple iCloud, Infomaniak kSuite, GMX, WEB.DE, mailbox.org, and read-only iCal feeds. Fastmail, Proton Calendar, Yahoo, Nextcloud and Posteo are listed as planned rather than live. GMX, WEB.DE and mailbox.org are common German consumer mailboxes.
...
On paid plans Kalender Sync markets Google and Microsoft updates as "Echtzeit", real time. Their documentation is more precise, describing those two as "nahezu sofort", nearly instant.
...
All their plans come with a 14-day trial of Business features and no card, and their AVV is included with Business and available on request otherwise.
...
**Any CalDAV server.** Covered above.
...
## What their comparison gets wrong about Keeper.sh Free

Their page describes Keeper.sh Free as a 30-minute sync, which is right about half of it. We read your calendars every minute on every plan, including Free. The 30 minutes is how often the free plan writes the copies out, and on Pro both halves run every minute.

Everything else on their page matches what we ship, including their description of self-hosting.

## What Kalender Sync has that Keeper.sh does not

**Paperwork.** They offer a signed AVV under Art. 28 DSGVO from Munich's PPJ Venture Labs UG (HRB 308771); Keeper.sh publishes no AVV or DPA for the hosted service.

**German hosting.** Databases at Hetzner in Falkenstein, with tokens encrypted at rest.

**Teams.** A quote-based Team plan from five users adds central management, single sign-on, audit logs and invoiced billing.

**Speed and Sharings.** Covered above.

## Running Keeper.sh yourself

Every Pro feature turns on automatically on your own instance, including per-minute writes and the uncapped API. The calendar data never leaves the server you control, which is how some organisations prefer to settle the paperwork question.
...
Keeper.sh, for $5 flat however many calendars you connect, event titles stripped by default, and Fastmail working today where theirs is still on a roadmap. Kalender Sync has the signed German data agreement, sharing into other people's calendars, a team plan and a German-language product.
