    ...
    <div className="flex flex-col gap-1.5">
      ...
      {isPullCapable && (
        <>
          ...
          />
          <DestinationsSection accountId={accountId} calendarId={calendarId} />
        </>
      )}
...
function DestinationsSection({ accountId, calendarId }: { accountId: string; calendarId: string }) {
  const { data: allCalendars } = useSWR<CalendarSource[]>("/api/sources");
  const { data: entitlements } = useEntitlements();
  ...
  const selectedDestinations = useMemo(
    () => pushCalendars.filter((calendar) => destinationIds.has(calendar.id)),
    [destinationIds, pushCalendars],
  );
